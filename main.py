...
BATCH_GRAPH_LOSS_TYPES = frozenset({"batch_graph"})


def is_batch_graph_loss(training_args: TrainingArguments) -> bool:
...
def main():
    ...
    use_batch_graph_loss = is_batch_graph_loss(training_args)
    ...
    if is_main_process():
        ...
        logger.info(f"  Batch graph k = {training_args.batch_graph_k}")
        logger.info(f"  Batch graph num_eigen = {training_args.batch_graph_num_eigen}")
