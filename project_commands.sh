...
train_segd_fastvlm() {
  _activate
  local LORA_R=32 LORA_A=64 BATCH_SIZE=16 EXP_NAME="SEGD_FastVLM_cls_r32_bs16_ckalast"
  torchrun --standalone --nproc_per_node=1 main.py \
    --model_name "apple/FastVLM-0.5B" \
    --teacher_model_name "raghavlite/B3_Qwen2_2B" \
    --lora True --teacher_lora True --lora_r $LORA_R --lora_alpha $LORA_A --teacher_lora_r 8 \
    --teacher_pooling "eos" --teacher_backbone "qwen2_vl" --model_backbone "llava_qwen2" --pooling "eos" \
    --dataset_name "TIGER-Lab/MMEB-train" --subset_name "ImageNet_1K" "N24News" "HatefulMemes" "VOC2007" "SUN397" --dataset_split "original" \
    --image_dir "vlm2vec_train/MMEB-train" --percent_data 1.0 \
    --output_dir "training/$EXP_NAME" \
    --per_device_train_batch_size $BATCH_SIZE --gradient_accumulation_steps 1 \
    --learning_rate 1e-4 --num_train_epochs 1 --bf16 --save_total_limit 2 \
    --logging_steps 5 --save_strategy "epoch" --seed 42 --weight_decay 0.01 \
    --normalize True --teacher_normalize True --lr_scheduler_type "cosine" --warmup_ratio 0.03 \
    --kd_loss_type "segd_loss" --kd_weight 1.0 --w_loss_cka 1.0 --cka_pooling "last" \
    --w_loss_v 1.0 --w_loss_t 0.7 --w_loss_cross 1.0 --w_loss_grounding 0.5 \
    --w_loss_grounding_warmup_ratio 0.15 --sekd_grounding_temp 0.1 \
    --knn_neighbors 10 --sekd_k_min 2 --sekd_k_max 16 --sekd_eig_eps 1e-6 \
    --sekd_align_grid_h 10 --sekd_align_grid_w 10 --image_resolution "low" \
    --report_to "none" --run_name "$EXP_NAME"
}
